...
- [x] 3G — Validation + version bump to 2.2.3. CURRENT_VERSION in src/main.js bumped 2.2.2 → 2.2.3. package.json version bumped 2.2.2 → 2.2.3. All validators clean: node --check src/main.js, node --check src/preload.js, scripts/_check-html-js.js, scripts/_validate-pool-worker.js (all three templates parse), scripts/_test-coordinator.js (49/49). Branch v2.0.0-elastic tagged v2.2.3.

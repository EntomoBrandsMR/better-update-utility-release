...
- [x] 2H — validation pass and version bump. CURRENT_VERSION in src/main.js bumped 2.2.1 → 2.2.2. package.json version bumped 2.2.1 → 2.2.2. All validators clean: node --check src/main.js, node --check src/preload.js, scripts/_check-html-js.js, scripts/_validate-pool-worker.js (all 3 templates), scripts/_test-coordinator.js (49/49). Branch v2.0.0-elastic tagged v2.2.2.
